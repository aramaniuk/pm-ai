...
- An overdue commitment with no telemetry coverage for its window resolves to `[UNKNOWN]`, never `[BROKEN]`. Absence of data is not evidence, and pre-meeting inquiries cannot be recalled.
- All Markdown in every scope is **plaintext by design** — the PM can read, grep, diff, and hand-edit their own record without the system's cooperation. The encrypted set is defined and closed; derived state is unencrypted, 600-permissioned, and rebuildable.
...
- The guard runs on **every capture location**, keyed on whether the path lies inside a git working tree rather than on which scope owns it. `<repo>/.project-ai/transcripts/`, `~/.pm-ai/private/people/<person_id>/transcripts/`, and `~/.manager-ai/transcripts/` are equally protected: encrypted at rest, and excluded from version control wherever version control can reach them. Tracked, or unanswerable, refuses the write — unknown is not permission. Outside a working tree there is nothing to be excluded from and the write proceeds. Losing a transient capture is recoverable; publishing verbatim minutes to a repository is not.
- The `git` CLI is a **hard runtime dependency of the capture write path**, not a developer convenience, and `pm-ai doctor` probes it because its absence refuses every capture with no other symptom. A libgit2 binding is not substitutable: its ignore check carries permanent `--no-index` semantics and reports an already-committed capture directory as protected.
...
- Two rules give opposite instructions for the same silence. An overdue commitment with no telemetry coverage resolves to `[UNKNOWN]` and never `[BROKEN]` (CAP-33, CAP-34); separately, an unhealthy connector's absence must never contribute "an `[UNKNOWN]` that looks like patience" — and names no replacement verdict. One compliant reading waits forever, another fires CAP-26's irreversible pre-meeting inquiry. Needs a third verdict or an explicit precedence.
