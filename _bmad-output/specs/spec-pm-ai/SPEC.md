...
- **Scope is decided by subject, not convenience.** A meeting record and its transcript live in the scope that owns the meeting, and a committed record may cite only a meeting in its own scope.
- The daemon verifies the `transcripts/` gitignore rule before writing a capture and refuses if it is absent. Losing a transient capture is recoverable; publishing verbatim minutes to the team's repository is not.
