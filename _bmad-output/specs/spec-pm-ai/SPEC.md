...
- How is the encryption debug toggle set by a user, and should a flag that writes credentials in plaintext be settable *persistently* at all? The spine puts it in `~/.pm-ai/config.toml`; nothing reads that file today and no entry point exists to configure, so this is blocked on the daemon and CLI rather than open by choice. The hazard is that a persistent setting is forgotten: the console warning scrolls away and the event-log entry fires only at startup. Candidate answers are a re-announcement interval, or an expiry after which the daemon refuses to start with the flag still set.
