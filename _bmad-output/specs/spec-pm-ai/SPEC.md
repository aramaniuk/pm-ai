...
- A coverage window is a **receipt for a request that reached the provider and was answered**, timed by pm-ai's own clock either side of that request — never a span computed from the clock alone. An empty answer is a receipt: asking and being told nothing is there is having looked. There is no window when the request failed (recorded as a failed attempt), when nothing that came back had a readable time, when everything that came back was rejected as unreadable, or when this machine's clock stepped backwards during the request. So *never looked*, *looked and failed* and *looked and found nothing* stay distinguishable — the distinction all three verdicts rest on.
...
- **Tier-1 deletion is authorised by a completed replacement and recorded where it cannot be deleted.** Compaction is the only operation that destroys truth: it deletes a sealed event-log segment only after the milestone summary replacing it is written and confirmed present, and records the deletion in the open segment of the same scope's log, naming each replaced segment and its checksum. A milestone summary carries forward the compaction records of the segments it replaces, or the audit trail is the first thing compaction erases.
- **A write becomes visible only when it is complete, and completeness is never inferred from elapsed time.** A capture is staged in `transcripts/temp/` and linked to its final name atomically, so the name's appearance and the content's completeness are one event; a whole-file replacement is staged and renamed, so a crash mid-rotation cannot destroy a credential store that AES-GCM makes unreadable when truncated. Appends cannot be atomic and do not need to be: a record without its terminating newline is not a record. No debounce interval is load-bearing for correctness.
- **Every derived artifact is produced by a declared job, and nothing enters Tier 3 that a job cannot rebuild from Tier 1 alone.** A job takes named inputs, produces named outputs, and does one thing; the dependency graph is *derived* from those declarations rather than configured, so a producer and its consumer cannot disagree about what makes what stale. Jobs are rows in the durable queue, never in-memory timers, and are woken three ways: a change to a watched artifact, a schedule for work no file change can announce, or a direct request from pm-ai's business logic. `inputs()` declares what makes a job stale, not what it opens.
- **Derived state is invalidated by one mechanism: system-wide filesystem notifications.** pm-ai does not publish change events from its own write path, even though it is the single writer, because Markdown is hand-editable by design and a second pipeline for the same question can disagree with the first. The OS sees every writer with one mechanism; the cost is a coalescing window per watched path, since one save arrives as several events. Only Tier-1 and `RETENTION_MANAGED` artifacts are eligible to be watched, which keeps every Tier-3 index out and so stops an indexing job re-triggering itself; one watcher owns each watched artifact, so a change cannot arrive twice or fall between two owners. A job must not write what re-triggers it — documented, not enforced. Writes made while the daemon was down are recovered by reconciling every watched path at boot — not periodically, which would be the excluded second pipeline — with the watcher attached first, so the overlap costs duplicate work rather than a silent miss. Every index entry carries the MD5 of the source file it derived from, held inside the index itself: that makes the re-index unit one source file rather than the whole index, makes a source deleted while the daemon was down detectable at all, and means the Tier-3 drop takes the checksums with it instead of leaving a rebuilt index believing it is current.
