...
**The lazy key fetch was ratified 2026-08-24, and the reason it is safe is operational rather than technical.** The frozen Approach said wiring *"fetches the key… and passes it"* — eager. Built that way it refused to boot any machine with no key enrolled, so it was changed during implementation, and that deviation was flagged rather than buried. The user kept it, and added the requirement that closes the gap it opened: **the key is configured before pm-ai is first run.** So in practice the daemon almost always has one, and lazy is the safety net rather than the normal path — the invariant eager would have enforced by refusing to start is now held by the setup order instead.

What lazy costs, named so it is not rediscovered: any operation writing both secret and non-secret state can now fail halfway. The mitigation is ordering — **write the secret first**, since a refused encrypted write leaves nothing behind — and it is recorded as a dispatch note on stories 5 and 8, the two that will hit it. Story 4 gained ownership of the enrolment command, which no story had.
