...
- Encryption and rebuildability are independent properties. `telegram_cache/` is encrypted *and* retention-managed; the vector index is plaintext *and* rebuildable; `personal_analytics.db` is plaintext *and* not rebuildable (example updated 2026-08-28 — it named `personal_analytics.db` as the encrypted case, stale against the 2026-08-23 narrowing). Do not infer either from the other.
...
| `~/.pm-ai/private/people/p1/dossier.md` | **yes** (fail-closed) | undeclared in the PEOPLE tree — members are runtime names — so the classifier fails closed; the intended *no* (gitignored, 600-permissioned, a single deletable directory) takes effect only when the story that writes dossiers declares the node (amended 2026-08-28) |
| `~/.manager-ai/private/telegram_cache/state.json` | yes | the PM's own voice notes and dialogue state |
| `~/.manager-ai/private/personal_analytics.db` | **no** | narrowed 2026-08-23 with the rest of the encrypted set: its protection is the gitignored enclave and file permissions (row amended 2026-08-28; it still said *yes* after the narrowing) |
...
**Two frozen rows amended on 2026-08-28, by review rather than by rename.** The story-1 code review found the matrix contradicting shipped behaviour twice. (a) The dossier row said *no* while the fail-closed rule this same story mandates makes an undeclared PEOPLE-tree path answer *yes* — the conflict was in the story from the start, and the row now records the fail-closed answer as current with the *no* as the declared-state target. (b) The `personal_analytics.db` row still said *yes* after the 2026-08-23 narrowing moved it out of the encrypted set; code follows `storage-contract.md`, and the row now agrees. The same review re-keyed `ENCRYPTION` on qualified relative keys (`private/telegram_cache/`, not `telegram_cache/`) so a file sharing a declared artifact's basename can no longer inherit its answer from elsewhere in the scope; the fail-closed rule and every row's verdict are unchanged by that re-keying.
