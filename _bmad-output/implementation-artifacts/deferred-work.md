...
- source_spec: `_bmad-output/specs/spec-pm-ai/stories/11a-meeting-records-reach-tier-one.md`
  summary: A meeting whose `start` moves to another UTC day cannot be rewritten in place, because `StoragePort` has no delete and the record's filename carries the day. `MeetingRecords.put` refuses with `MeetingDisplaced` rather than duplicating or stranding the record.
  evidence: The name is `<utc-day>-<slug>-<sha256 of the id>.md`, and the day is what lets `for_day` read one day's records without opening every file — which is the whole of the matrix row "one bad file, unrelated date; `for_day` reads only its own day; unrelated damage does not raise". Publishing the new name leaves the old file behind, so one id would have two records and `get` would have to pick; keeping the old name means `for_day` never finds the meeting again, a wrong answer rather than an error. Neither is acceptable, so the write is refused and names the member to remove. Unreachable through `run_transcript_ingestion`, which writes each meeting once; it becomes live in `33c`, where a rescheduled calendar event is ordinary. The real fix is a delete or rename on the single writer — the first artifact operation that needs one, since every ledger is append-only and every other artifact is replaced whole.
...
- source_spec: `_bmad-output/specs/spec-pm-ai/stories/33c-graph-calendar-mapping.md`
  summary: `wait=time.sleep` is injected into `GraphClient` with no cap, so a provider-controlled `Retry-After` decides how long a harvest blocks.
  evidence: The injection is 33c's — it is the composition `33b` deliberately deferred, and pinning it is one of this slice's review patches. The bound is not: `GraphClient` owns the wall-clock budget that stops a walk, and whether that budget is checked before or after the sleep is a property of `33b`'s client rather than of this call site. Recorded here because 33c is where a real `time.sleep` first reaches that code at all, so a header that was inert until now became live in this slice. `min(seconds, <cap>)` at the injection is the cheap fix if the client's budget turns out not to cover it; confirming which is true is the first step and belongs with whoever next opens `client.py`.
