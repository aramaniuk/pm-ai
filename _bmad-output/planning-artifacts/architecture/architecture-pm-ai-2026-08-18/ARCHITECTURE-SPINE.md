...
- **Rule:**
  ...
  - **Coverage is recorded when there is coverage to record** `[revised 2026-09-23]`. A harvest returns a `CoverageWindow` **or `None`**, and `None` is a real answer rather than a gap to paper over — a mandatory field is what drove a connector to fabricate a window from the clock, which this very guard then read as evidence. A window is refused outright from a harvest that returned no rows, and a harvest that failed is durably recorded as having failed, so "ran and learned nothing" survives the process as something other than silence. The commitment sweeper must not declare `BROKEN` across a window it has no coverage for. Silence from a sleeping laptop is missing data, not evidence of a broken promise — and FR-26's nudges are irreversible, so this must fail closed.
...
| **The daemon is one project wide; the architecture says it spans many** `[2026-09-24]` | AD-10 gives *each registered project* independently-scheduled harvesting and AD-19 puts all I/O on **one** loop — together, one daemon over many project-scoped connector instances. But `wiring.build(root, project: str)` binds a daemon to a single project, so `entry._compose` refuses at `len(projects) > 1` and reports the refusal as a `FAILING` probe | **Enrolling a second project — the intended steady state — is reported as a fault, and its telemetry is never harvested.** Two halves, and only one is undecided: *which scope a CLI command acts in* is already answered by AD-11 ("the CLI, when run inside a registered repository, binds to that project scope") and is merely unimplemented — `Path.cwd()` appears once in the package and only to absolutise a `project add` argument. *Which projects get harvested* is the open one, and there selection is the wrong fix: picking one winner leaves the rest unharvested, which is what AD-10 forbids. The daemon has to span projects, or AD-10 and AD-19 have to say something else |
| **`setup` and `doctor` disagree about the same machine** `[2026-09-24]` | Both end in a probe report, but `setup` calls `run_all()` alone while `doctor` calls `_diagnose()`, which appends the composition failure. With two projects registered, measured: `setup` reports healthy and exits 0; `doctor` reports `FAILING` and exits 4. The dedup in `_diagnose` is keyed on probe name and these differ (`project` vs `project registry`), so nothing suppresses it | An operator who runs `setup`, sees green, then runs `doctor` and sees red has no way to reconcile them. `_setup`'s own docstring says its final claim is "the probe report and not 'the steps succeeded', because every step can succeed on a machine that is still not ready" — and then narrows the report so that the one probe contradicting it is excluded, reintroducing exactly the failure the docstring guards against. Both halves were deliberate and documented; neither author saw the other's consequence. Symptom of the row above, and it should not be fixed by widening `setup` alone while the daemon stays single-project |
| **A quiet project's commitments can never be verified broken** `[2026-09-23]` | Coverage is refused from any harvest that returned no rows — but `EMPTY` is documented as "the provider answered and there was nothing there", which is a *completed fetch*, and coverage is exactly what arms the fail-closed reading that makes `BROKEN` reachable. On a quiet project every harvest is `EMPTY`, so the sweeper never sees a covered window | `FULFILLED` stays reachable while `BROKEN` does not, so the verifier silently only ever delivers good news. The 8a repair conflated two different empties: a window fabricated from the clock, which earns nothing, and a fetch that reached the provider and found nothing, which has earned its window. Separating them is the fix |
