...
### AD-6 — Scoped encryption, keychain-held key, no persistent off switch `[revised 2026-08-27, 2026-09-25]`
...
- **Rule:** **Exactly two artifacts are encrypted:** `~/.pm-ai/private/config.json` (credentials) and `~/.manager-ai/private/telegram_cache/` (the PM's own voice notes and dialogue state). Everything else on disk is plaintext at `0600` inside `0700`, under FileVault.
  ...
  **There is no persistent way to disable encryption.** No `config.toml` key, no stored debug profile, no CLI flag that survives a restart. The whole mechanism is the `PM_AI_DISABLE_ENCRYPTION` environment variable, which dies with the process — so **restarting restores encryption unconditionally**, with no expiry mechanism, no re-announcement schedule, and nothing to audit. A console warning scrolls away within minutes and an event-log entry is weeks old by the time anyone wonders why a credential file is readable, which is why a durable switch is not an acceptable substitute. Parsed against an explicit allowlist, never truthiness: `=0` reads to a human as *off* and must not read as *on*. Read in exactly one place, `pm_ai.platform.environment`, because two callers need the same answer — the composition root, which acts on it, and `pm-ai doctor`, which reports it. Off is never the state of a fresh install.

  **When off, the two announcements happen at different moments** `[2026-09-25]`, and **both belong to the process that owns the writer** — the one that reads the flag and chooses the plaintext cipher, never a client driving it. The console warning prints when that process composes, because it costs nothing permanent. The `event_log/` entry — `security`, application scope — is written **just before a writer's first protected write in plaintext**, never at start-up and never on a read, so the append-only log does not gain a line for every `--help` or mistyped command. **If the entry cannot be written, the protected file is not written either**: no protected file is written in plaintext without an entry first. One entry per writer, not per process — the already-recorded fact lives on that writer's cipher, never in a module global (AD-30) — and every writer the composition root builds is bound this way.

  **The entry records a writer's state, not a file.** It says a writer ran with encryption off and was about to write a protected file; it names no file, and a publish can still fail after it. Whether plaintext protected files exist is answered by examining the files, never by reading the log.
...
### AD-11 — Projects are registered explicitly `[revised 2026-09-25]`

- **Binds:** CLI, Telegram bridge, FR-35
- **Prevents:** filesystem scanning silently opting a repository into telemetry harvesting; two commands deciding which project they act in by different orders
- **Rule:** Projects enter the system only via `pm-ai project add`; the registry lives in `~/.pm-ai/`. No auto-discovery. The CLI, when run inside a registered repository, binds to that project scope, under the selection order below; Telegram has no working directory and requires explicit project selection or a configured default. Connector configuration is reachable from both surfaces and operates on the one registry through the daemon.
- **The folder rule is implemented** `[2026-09-24, story 4l]`. `entry._acting_project` matches the working directory against each enrolled repository **by containment, innermost first**, and asks git nothing — a project is onboarded at a directory and is not required to be a repository, so `working_tree` would refuse the command from inside a perfectly good project. A single enrolled project binds from any directory at all. Outside every enrolled project with more than one enrolled, the command is **refused** — naming each project and its directory — rather than bound to a guess. Three states refuse and each composes its own sentence: outside them all, one directory registered under two ids, and a working directory that cannot be read. None of the three is a fault, so `pm-ai doctor` carries a `project selection` probe that is **always `OK`** — it says which project this invocation binds to, or that the directory names none, without letting the working directory decide an exit code. This is the half of AD-11 that was prose until 4l: `Path.cwd()` appeared once in the package, and only to absolutise a `project add` argument.
- **Selection order for a CLI command** `[2026-09-25]`. A command that acts in a project acts in the first of:
  1. **a project named on its command line**;
  2. the enrolled project whose folder contains the working directory, innermost first;
  3. the only enrolled project, from any directory.

  Otherwise it is refused, naming each enrolled project. The rules around the order:
  - **The named project is the project the command acts in**, not only its output target. Every write the command makes and the cross-scope citation guard (AD-38) read it, so a command never holds a target in one project and an acting project in another.
  - **A named project wins over the folder** when they disagree. The name is settled before the working directory is read, so it lifts all three folder refusals — outside every folder, one folder under two ids, an unreadable working directory.
  - **A named project that is not enrolled is refused**, never passed on to the folder or the single-project rule.
  - **The command line is parsed once**, and selection runs on that parsed result, never on a second reading of `argv`.
  - **One spelling:** `--scope project:<id>`, the form `pm-ai dashboard` already parses, naming the project's id. No second flag names the same thing.
  - **A command whose target is not a project needs no selection** and is never refused for the lack of one. The personal dashboard, `goal set`, and anything else aimed at personal or application scope run from any directory.
  - **The folder decides which project a command acts in, never a command's default target.** `pm-ai dashboard` with no `--scope` renders the personal dashboard (CAP-9) wherever it runs.

  **Not yet built** `[2026-09-25]`. Today the project is chosen before the command line is read. So a named project neither wins nor lifts a refusal, and with several projects enrolled, commands aimed at personal scope are refused outside every folder — the personal dashboard among them. The slice that builds this order also fixes the refusal sentence ("no way to name a project on the command line yet") and `doctor`'s `project selection` line, which always credits the working directory.
...
| Name | Version | Notes |
| --- | --- | --- |
| Python | **3.14 exactly** — `requires-python = ">=3.14,<3.15"`, `.python-version` `3.14` | Must be a **uv-managed** interpreter — see the extension-loading prerequisite below. Exact because the prompt-injection filter's recorded rule fingerprint includes Python's Unicode tables (16.0.0 on 3.14, 15.1.0 on 3.13), so any other minor version fails its fingerprint test. Moving to 3.15 is its own slice: new Unicode data is a new rule, so the filter's rates are re-measured first. 3.15.0 is due 2026-10-01 and ships Unicode 17.0.0 (measured on 3.15.0rc1), so that move needs a new rule version; 3.14 receives bugfix releases until October 2027 |
...
| `watchdog` (FSEvents observer) | **`==6.0.0`**, verified on PyPI 2026-08-27 and still the latest on 2026-09-25, declared 2026-08-28 | AD-46's `FileWatcherPort` adapter. **No macOS wheel for Python 3.14** `[2026-09-25]` — its macOS wheels stop at 3.13, so on the one v1 platform the runtime extra builds it from source, which needs the Xcode Command Line Tools (verified: a source build succeeds and the FSEvents backend loads; an install that forbids source builds fails). Latest release is 2024-11-01, so ~23 months without one — a standing currency risk of the same shape as `sqlite-vec` below, though a far smaller surface. macOS FSEvents and kqueue backends; requires Python ≥3.9 against this project's 3.14. AD-46 deliberately does not depend on historical replay from a persisted event id, so no backend-specific capability is load-bearing here. |
...
- **Whether compaction carries AD-6's `security` entries forward.** `[2026-09-25]` AD-5's compaction record names what it removed, but an entry explaining a plaintext protected file can be compacted away while the file remains. Decide with compaction itself: carry `security` entries into the summary, or accept that the files — not the log — are the authority, as AD-6 already states. Nothing compacts today.
- **Where Telegram's configured default project lives, and whether the CLI reads it.** `[2026-09-25]` AD-11 gives Telegram "explicit selection or a configured default" and the CLI no default. The CLI rejected a default because a forgotten `cd` would silently act in it. Decide with the Telegram bridge (story 5): if the default is stored where the CLI can read it, say that the CLI never does, or AD-7's same-functionality rule and the CLI's order disagree.
...
| ~~**Composition is eager, so a diagnostic command mutates Tier 1**~~ `[2026-09-24]` **Closed 2026-09-25 by story 1p** | `_compose` runs before `argv` is inspected, so any invocation builds the whole daemon. With encryption disabled and one project registered, building it appended a `security` entry to the append-only application `event_log/` — five `pm-ai --help` runs and one invalid command produced five lines | Composition is still eager, but building no longer writes to Tier 1: the entry moved to the first protected write (AD-6). Re-measured 2026-09-25 on a machine already in use — `--help`, a mistyped command, `project` and `doctor` change no file; on a fresh home the first command still creates `operational.db` (Tier 2). Encoded in `tests/architecture/test_cipher.py` |
...
- **Two writers choose the plaintext cipher without printing the console warning** `[2026-09-25]`. `pm-ai setup`'s writer and project enrolment's are built outside `build()`, where the warning lives. Both record the event-log entry correctly, and neither writes a protected file today, so the warning is the only thing missing. And `pm-ai setup` builds a new writer per step, so "one entry per writer" is bounded by how many writers a command builds rather than by anything stated. Close both when the daemon owns a writer for its lifetime (AD-7).
- **`watchdog` needs a source build on macOS under 3.14** `[2026-09-25]`. The same shape of risk that removed SQLCipher on 2026-08-27, on a smaller surface: the Xcode Command Line Tools become an install prerequisite for the runtime extra until `watchdog` ships a 3.14 macOS wheel. Nothing has installed the extra yet.
